...
function slideMasterRelsXml() {
  // A slide master must declare its own relationship to a theme (PowerPoint resolves
  // it via this part relationship, not via any in-content reference) -- omitting it
  // produces a package PowerPoint rejects outright as corrupted, even though a theme
  // relationship also exists on presentation.xml.rels.
  return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n' +
    '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">' +
...
// Every EMU coordinate in DrawingML is an xsd:long. A fractional value (which is
// easy to produce by dividing a slide width into columns) is schema-invalid, and
// PowerPoint rejects the whole package as corrupt rather than rounding it itself.
// Every shape emitter below coerces its geometry through this.
function emu(v) { return Math.round(v) || 0; }
...
function slideRelsXml(imageRels) {
  ...
  // Every slide MUST carry a relationship to its slideLayout -- PowerPoint resolves a
  // slide's layout purely through this part relationship (nothing in slide.xml itself
  // references it), so omitting it produces a package PowerPoint flatly refuses to open
  // ("file or directory is corrupted and unreadable") even though the ZIP/XML are all
  // individually well-formed. Fixed high id so it never collides with the per-slide
  // image rIds (which start at rId1).
  return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>\n' +
    '<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">' +
    '<Relationship Id="rId1000" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>' +
...
// Four viewpoints across a 16:9 slide is about the limit before each photo gets too
// small to judge; beyond eight pairs the extras are dropped rather than shrunk.
var MAX_PAIRS_PER_SLIDE = 4;

// One slide of before/after PAIRS. Each column is a single photographic viewpoint:
// the pre-op frame on the top row, the matching post-op frame directly beneath it,
// so the comparison reads down the column instead of across two separate slides.
//
// `pairs` is [{ before, after }, ...]; either half may be null when the two sets are
// of unequal length, in which case that cell shows an empty placeholder card rather
// than shifting the remaining photos out of alignment with their partners.
function photoPairSlide(label, pairs, mediaStartIndex) {
  var margin = 450000;
  var titleH = 750000;
  var gutter = 150000;
  var labelW = 820000;      // left gutter carrying the BEFORE / AFTER row labels
  var cols = pairs.length || 1;
  var gridTop = margin + titleH;
  var gridLeft = margin + labelW;
  var usableW = SLIDE_W - margin * 2 - labelW;
  var usableH = SLIDE_H - gridTop - margin;
  var cellCx = (usableW - gutter * (cols - 1)) / cols;
  var cellCy = (usableH - gutter) / 2;
  ...
  shapes.push(textBoxShape(shapeId++, 'Label', margin, margin, SLIDE_W - margin * 2, titleH, label,
    { size: 2800, bold: true, color: THEME.primaryDark, align: 'l' }));

  // Row labels, one per phase.
  ['Before', 'After'].forEach(function (rowName, row) {
    var rowY = gridTop + row * (cellCy + gutter);
    shapes.push(textBoxShape(shapeId++, 'Row' + rowName, margin, rowY, labelW - 60000, cellCy, rowName,
      { size: 1600, bold: true, color: row === 0 ? THEME.textMuted : THEME.primaryDark, align: 'r' }));
  });

  pairs.forEach(function (pair, col) {
    var cellX = gridLeft + col * (cellCx + gutter);
    ['before', 'after'].forEach(function (phase, row) {
      var cellY = gridTop + row * (cellCy + gutter);
      // Card backdrop so the grid reads cleanly even with varying aspect ratios.
      shapes.push(rectShape(shapeId++, 'Card' + col + phase, cellX, cellY, cellCx, cellCy, THEME.surface));
      ...
      var rId = 'rId' + (imgCount + 1);
      var mediaName = 'image' + (mediaStartIndex + imgCount) + (img.ext === '.png' ? '.png' : '.jpeg');
      rels.push({ rId: rId, target: '../media/' + mediaName });
      media.push({ name: 'ppt/media/' + mediaName, data: img.buffer });
      shapes.push(pictureShape(shapeId++, 'Photo' + col + phase, rId, fit.x, fit.y, fit.cx, fit.cy));
      imgCount++;
    });
  });
...
// Pair the two sets up by position: both are held in capture order, so the nth
// pre-op frame and the nth post-op frame are the same viewpoint. A longer set simply
// leaves the missing half of its trailing pairs empty.
function pairPhotos(beforeImages, afterImages) {
  var n = Math.max(beforeImages.length, afterImages.length);
  var pairs = [];
  for (var i = 0; i < n; i++) {
    pairs.push({ before: beforeImages[i] || null, after: afterImages[i] || null });
  }
  return pairs;
}
...
//
// Three slides: a poster, then the before/after pairs split across two photo slides.
// The pairs are NOT grouped by phase -- each column holds one viewpoint with its
// pre-op frame above its post-op frame, so the surgeon and patient read the change
// down a column instead of flipping between a "before" slide and an "after" slide.
function buildBeforeAfterPptx(opts) {
  opts = opts || {};
  var poster = posterSlide(opts.title || 'Feather Touch', opts.subtitle || '', opts.meta || '', opts.logo);

  var pairs = pairPhotos(opts.beforeImages || [], opts.afterImages || []);
  // Split as evenly as possible so two slides of three beats a slide of five and a
  // slide of one; cap each slide at four columns to keep the photos legible.
  var perSlide = Math.min(MAX_PAIRS_PER_SLIDE, Math.max(1, Math.ceil(pairs.length / 2)));
  var firstPairs = pairs.slice(0, perSlide);
  var secondPairs = pairs.slice(perSlide, perSlide * 2);

  var slideA = photoPairSlide(
    secondPairs.length ? 'Before / After (1 of 2)' : 'Before / After',
    firstPairs, 100);
  var slideB = photoPairSlide('Before / After (2 of 2)', secondPairs, 200);
  ...
  var files = [
    ...
    { name: 'ppt/slides/slide2.xml', data: Buffer.from(slideXml(THEME.bg, slideA.shapesXml), 'utf8') },
    { name: 'ppt/slides/_rels/slide2.xml.rels', data: Buffer.from(slideRelsXml(slideA.rels), 'utf8') },
    { name: 'ppt/slides/slide3.xml', data: Buffer.from(slideXml(THEME.bg, slideB.shapesXml), 'utf8') },
    { name: 'ppt/slides/_rels/slide3.xml.rels', data: Buffer.from(slideRelsXml(slideB.rels), 'utf8') }
  ];
  poster.media.forEach(function (m) { files.push(m); });
  slideA.media.forEach(function (m) { files.push(m); });
  slideB.media.forEach(function (m) { files.push(m); });

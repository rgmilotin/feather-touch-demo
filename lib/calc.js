...
(function (root, factory) {
  ...
  var DEFAULT_MEAS = {
    ...
    dimd: '5',           // current cleavage / inter-mammary distance (midline, single)
    ...
    desiredImd: '3.5',          // desired postoperative intermammary distance
  ...
  // Current Cleavage is recorded as one of these presets (cm).
  var CLEAVAGE_OPTIONS = [4, 5, 6, 7, 8];

  // Desired postoperative intermammary distance: the slider's range, step and the
  // mid-scale value the implant-width adjustment is anchored to.
  var IMD_MIN_CM = 2.5;
  var IMD_MAX_CM = 4.5;
  var IMD_STEP_CM = 0.2;
  var IMD_REFERENCE_CM = 3.5;
  ...
  function normaliseMeas(meas) {
    ...
    // The Current Cleavage presets used to start at 3 cm; snap anything off the
    // current 4-8 cm scale onto the nearest option so the radio group always has a
    // selected value and the width formula keeps a sane gap.
    var gap = toNum(m.dimd, CLEAVAGE_OPTIONS[0]);
    if (CLEAVAGE_OPTIONS.indexOf(gap) === -1) {
      m.dimd = String(CLEAVAGE_OPTIONS.reduce(function (best, o) {
        return Math.abs(o - gap) < Math.abs(best - gap) ? o : best;
      }, CLEAVAGE_OPTIONS[0]));
    }
  ...
  // Implant-width correction from the DESIRED intermammary distance.
  //
  // The base width available on one side of the chest is (chest width - gap) / 2, so
  // the two are inversely related: planning a narrower postoperative cleavage frees
  // medial room and admits a wider implant, and asking for a wider gap takes it away.
  // Half of any change in the gap falls on each breast, hence the 5 mm per cm.
  //
  // It is anchored at the mid-scale value rather than at the measured pre-op cleavage,
  // which keeps the correction inside +/-5 mm across the whole slider and leaves the
  // validated desktop-app formula untouched at the default. The influence is one-way:
  // nothing here feeds back into the desired IMD.
  var IMD_WIDTH_MM_PER_CM = 5;

  function imdWidthAdjustment(meas) {
    var imd = toNum(meas.desiredImd, IMD_REFERENCE_CM);
    if (!(imd > 0)) return 0;
    return (IMD_REFERENCE_CM - imd) * IMD_WIDTH_MM_PER_CM;
  }
  ...
  function autoWidth(meas, side) {
    ...
    w = Math.round(w + imdWidthAdjustment(meas));
    return w > 0 ? w : 0;
  }
  ...
  return {
    ...
    CLEAVAGE_OPTIONS: CLEAVAGE_OPTIONS,
    IMD_MIN_CM: IMD_MIN_CM,
    IMD_MAX_CM: IMD_MAX_CM,
    IMD_STEP_CM: IMD_STEP_CM,
    IMD_REFERENCE_CM: IMD_REFERENCE_CM,
    imdWidthAdjustment: imdWidthAdjustment,
